...
export const TicketNotification = ({ ticketId, status = 'PENDING' }: TicketNotificationProps) => {
  return (
    <motion.div
      ...
      className="glass-panel border-rose-500/30 bg-rose-500/5 p-4 rounded-2xl mb-6 relative overflow-hidden group"
    >
      <div className="absolute top-0 right-0 p-8 bg-rose-500/10 blur-2xl rounded-full" />
      ...
      <div className="flex items-start gap-4 relative z-10">
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between mb-1">
            <h3 className="text-sm font-bold text-white uppercase tracking-tight">Support Ticket Created</h3>
          ...
          <div className="flex items-center gap-3">
            <div className="flex-1 bg-black/20 rounded-lg px-3 py-2 border border-white/5 font-mono text-[11px] text-white/80">
              ID: {ticketId}
            </div>
            <button className="flex items-center gap-2 text-[11px] font-bold text-nebula-blue hover:text-white transition-colors group">
              View Ticket
